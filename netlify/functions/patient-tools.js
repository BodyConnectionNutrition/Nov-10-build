...
function expectedCookie() {
  const key = secret();
  return key ? crypto.createHmac("sha256", key).update("body-connection-patient-tools-v1").digest("base64url") : "";
}
...
function loginPage(error = "") {
  return shell(`<p>Patient access</p><h1>Tools shared with you</h1><p>Enter the patient access password provided to you by Body Connection Nutrition.</p>${error ? `<p class="error">${escapeHtml(error)}</p>` : ""}<form method="post" action="/patient-tools/login"><label for="password"><b>Patient access password</b></label><input id="password" name="password" type="password" required><button type="submit">Enter patient tools →</button></form>`, "Patient Tools");
}

function redirect(location, extraHeaders = {}) {
  return { statusCode: 302, headers: { Location: location, "Cache-Control": "no-store", ...extraHeaders }, body: "" };
}
...
exports.handler = async (event) => {
  ...
  if (file) {
    try {
      ...
      console.error(error);
      return { statusCode: 500, headers: headers(), body: shell("<h1>Tool unavailable</h1><p>Please try again shortly.</p>") };
    }
  }
